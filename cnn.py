...
#
#     def __init__(self, non_static, hidden_unit, sequence_length, max_pool_size, embedding_mat, embedding_category_mat,tficf,
...
#         print("{} : {}".format("emb.shape", np.shape(self.emb)))
#
...
#                  conv1 = self.conv2D(self.emb,W1,b1,'Conv1')
#
...
#         self.result = tf.nn.dropout(self.pooled_concat, self.dropout_keep_prob)
...
#
#         if tficf:
#             W_emb = tf.constant(embedding_category_mat,name="W_emb",dtype=tf.float32)
#             fc_concat = []
#             fc_concat.append(self.pooled_concat)
#             self.fc_input = tf.nn.embedding_lookup(W_emb,self.input_x)
#             print(self.fc_input)
#             print("{} : {}".format("fc_input.shape", np.shape(self.fc_input)))
#             fc_concat.append(self.fc_input)
#             fc_concat = tf.concat(fc_concat,1)
#
#             print("{} : {}".format("fc_concat.shape", np.shape(fc_concat)))
#
#
#             self.result = tf.layers.dense(inputs=fc_concat,units=num_filters*len(filter_sizes),activation=tf.nn.relu)
#
#             print("{} : {}".format("dense.shape", np.shape(self.result)))
#
#
...
#             # self.scores = tf.nn.xw_plus_b(self.result[:,-1], W, b, name='scores')
